...
function CartesianOPTSymbolics(exprs,fields,vars, exts = nothing ; trialFunctionsCharacteristics=(orderBtime=1,orderBspace=1, pointsUsed=[3,3,3,3]))
    ...
    *dt^(it-1)*dx^(ix-1)*dy^(iy-1)*dz^(iz-1)
    ...
    @show shape(newC_Coefs_deriv_generalised)
    

    newC_Taylor_generalised(dt,dx,dy,dz)=sum(sum(sum(sum(newC_Coefs_deriv_generalised[it,ix,iy,iz]*dt^(it-1)*dx^(ix-1)*dy^(iy-1)*dz^(iz-1)
    / factorial(BigInt(it-1)) / factorial(BigInt(ix-1)) / factorial(BigInt(iy-1)) / factorial(BigInt(iz-1)) 
    for it in 1:orderC[1]; init=0) for ix in 1:orderC[2]; init=0) for iy in 1:orderC[3]; init=0) for iz in 1:orderC[4]; init=0)

    ### Maybe it's worth preparing U_Taylor derivatives or integrals in advance too (but we don't know how integrals are applied for the moment)
    
    
    #@variables T X Y Z
    #newC_Taylor_generalised(T,X,Y,Z)

    #endregion

    #region STEP 4: expression for continuous material variables using discretised values based on Taylor expansions, naming material variables on nodes
    Vars=[]
    for iVars in 1:NtypeofMaterialVariables
        tmpTXYZ=[t, x, y, z] .* eachVariableDependency[:,iVars]
        tmpExpression=mySimplify(newC_Taylor_generalised(tmpTXYZ[1],tmpTXYZ[2],tmpTXYZ[3],tmpTXYZ[4]))
        ...
        # make a mapping dictionary for a specific material variable (var[iVars])
        ...
        tmpVarExpression=substitute(tmpExpression,Dict(specificMaterialMapping))
        #@show tmpVarExpression=substitute(tmpExpression,Dict(C_given_generalised[1:Stₗ+Stᵣ+1,1:Sxₗ+Sxᵣ+1,1:Syₗ+Syᵣ+1,1:Szₗ+Szᵣ+1]=>Λ[1:Stₗ+Stᵣ+1,1:Sxₗ+Sxᵣ+1,1:Syₗ+Syᵣ+1,1:Szₗ+Szᵣ+1]))
        Vars =push!(Vars,tmpVarExpression)
    end 

    #endregion

    #region STEP 5: translate the expressions into Taylor expanded fields
    ...
    #     tmpExpr=exprs[iExpr]
    #     newtmpExpr=expand_derivatives(substitute(tmpExpr,Dict(mapping)))
    #     newExpr=push!(newExpr,newtmpExpr)
    # end
       ...
       # @show deltaVariableArray=[Δt Δx Δy Δz] # we need think how to implement irregular grids
        for iComponent in 1:4        
            ...
            if orderBspline[iComponent] == 0
                ...
                newExpr[iExpr]=substitute(newExpr[iExpr], Dict([variableArray[iComponent]=>0]))
            elseif orderBspline[iComponent] == 1
                ...
                @time f1=integrateTaylorPolynomials(newExpr[iExpr],variableArray[iComponent])
                ...
                newExpr[iExpr]=mySimplify(desired_value)
    ...
    # here newExpr is the list of desired value (symbolic integration over the 'element')
    ...
    for iExpr in 1:NtypeofExpr
        difference = mySimplify(innerProducts[iExpr]-newExpr[iExpr])
    ...
    for iz in 1:Lzₗ+Lzᵣ+1
        for iy in 1:Lyₗ+Lyᵣ+1
            for ix in 1:Lxₗ+Lxᵣ+1
                for it in 1:Ltₗ+Ltᵣ+1
                    newA[it,ix,iy,iz,1,1]=mySimplify(newA[it,ix,iy,iz,1,1])
...
# If not the function will compute for 4D dependency
# In fact, the coefficients should be only a function of variableDependency
...
elseif iExecute == 1 
    ...
    CartesianOPTSymbolics(exprs,fields,vars)  # or CartesianOPTSymbolics(exprs,fields,vars,exts)
